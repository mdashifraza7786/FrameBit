...
export async function POST(
  ...
  try {
    ...
    const { allowed } = await verifyProjectAccess(asset.projectId.toString(), user._id.toString(), 'editor');
    if (!allowed) {
      return NextResponse.json({ error: 'Only editors and owners can create share links' }, { status: 403 });
    }
