    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-200">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-zinc-500 font-medium">
          <Link href="/projects" className="hover:text-brand-600 dark:hover:text-zinc-300 transition-colors">
            Projects
          </Link>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 rounded-3xl bg-white dark:bg-zinc-900/60 border border-slate-200 dark:border-zinc-800/80 backdrop-blur-sm shadow-xl">
          <div className="space-y-2 min-w-0">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-zinc-800 text-brand-700 dark:text-teal-400 border border-slate-200 dark:border-zinc-700 font-semibold uppercase tracking-wider shrink-0">
                {project.userRole}
              </span>
            </div>
            {project.description && (
            ...
            <div className="flex items-center gap-4 text-[11px] text-slate-400 dark:text-zinc-500 pt-1">
              ...
              <span className="flex items-center gap-1">
                <HardDrive className="w-3 h-3 text-brand-600 dark:text-teal-400" /> Stored in Google Drive
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            {canUpload && (
              <button
                ...
                className="px-4 py-2 rounded-xl bg-brand-600 hover:bg-brand-500 dark:bg-teal-600 dark:hover:bg-teal-500 text-white font-semibold text-xs shadow-lg shadow-brand-600/30 dark:shadow-teal-600/30 flex items-center gap-2 transition-all"
              >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-bold text-slate-800 dark:text-zinc-100 flex items-center gap-2">
              <Film className="w-4 h-4 text-brand-600 dark:text-teal-400" /> Video Assets ({videoAssets.length})
            </h2>
            ...
            {videoAssets.length > 0 && (canUpload || canChangeStatus) && (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 border transition-all ${
                  selectMode
                    ? 'bg-brand-600 hover:bg-brand-500 dark:bg-teal-600 dark:hover:bg-teal-500 text-white border-brand-600 dark:border-teal-600'
          ...
          {/* Bulk Action Bar */}
          {selectMode && (
            <div className="flex flex-wrap items-center gap-3 p-3 rounded-2xl bg-brand-50 dark:bg-teal-950/30 border border-brand-200 dark:border-teal-900/60">
              <button
                onClick={toggleSelectAll}
                className="text-xs font-semibold text-brand-700 dark:text-teal-300 flex items-center gap-1.5 hover:underline shrink-0"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {videoAssets.map((vid) => {
                ...
                <Link
                  ...
                  className={`group flex flex-col justify-between p-5 rounded-3xl bg-white dark:bg-zinc-900/80 hover:bg-slate-100 dark:hover:bg-zinc-800 border transition-all shadow-xl hover:shadow-brand-500/5 dark:hover:shadow-teal-500/5 space-y-4 ${
                    isSelected
                      ? 'border-brand-500 ring-2 ring-brand-500/40 dark:border-teal-500 dark:ring-teal-500/40'
                      : 'border-slate-200 dark:border-zinc-800/80 hover:border-brand-500/40 dark:hover:border-teal-500/40'
                  }`}
                >
                  {/* Thumbnail / Video Preview */}
                  <div className="aspect-video w-full rounded-2xl bg-slate-900 dark:bg-zinc-950 border border-slate-300 dark:border-zinc-800/80 flex items-center justify-center relative overflow-hidden group-hover:border-brand-500/40 dark:group-hover:border-teal-500/40 transition-all shadow-inner">
                    <img
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-black/30 opacity-60 group-hover:opacity-80 transition-opacity flex items-center justify-center">
                      {!selectMode && (
                        <div className="w-12 h-12 rounded-full bg-brand-600/90 dark:bg-teal-600/90 text-white flex items-center justify-center shadow-xl opacity-0 group-hover:opacity-100 group-hover:scale-110 transition-all duration-200">
                  ...
                  <div className="flex items-center justify-between gap-2 pt-1">
                    <div className="min-w-0 flex-1">
                      <h3 className="font-bold text-sm text-slate-800 dark:text-zinc-100 group-hover:text-brand-600 dark:group-hover:text-teal-400 transition-colors line-clamp-1">
                        {vid.name}
                      </h3>
            ...
            <div className="w-full max-w-md bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95">
              ...
              {isOwner && (
                <form onSubmit={handleInviteMember} className="space-y-3 p-4 rounded-2xl bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800">
                  <span className="text-xs font-semibold text-slate-700 dark:text-zinc-300 flex items-center gap-1.5">
                    <UserPlus className="w-3.5 h-3.5 text-brand-600 dark:text-teal-400" /> Add Collaborator
                  </span>
                  ...
                  <div className="space-y-2">
                    <input
                      ...
                      className="w-full bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-800 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-zinc-500 focus:outline-none focus:border-brand-500 dark:focus:border-teal-500"
                    />
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-brand-600 hover:bg-brand-500 dark:bg-teal-600 dark:hover:bg-teal-500 text-white rounded-xl text-xs font-semibold shadow-md disabled:opacity-50 transition-all shrink-0"
                      >
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                ...
                <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800/80">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-full bg-brand-500/20 dark:bg-teal-500/20 text-brand-700 dark:text-teal-300 font-bold text-xs flex items-center justify-center border border-brand-500/40 dark:border-teal-500/40">
                  ...
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-brand-50 dark:bg-teal-950/60 border border-brand-500/40 dark:border-teal-500/40 text-brand-700 dark:text-teal-300 font-semibold uppercase">
                    Owner
                  </span>
                </div>
